...
namespace HTTPServerLib
{
    public class HttpResponse : BaseHeader
    {
        ...
        protected string BuildHeader()
        {
            ...
            if (!string.IsNullOrEmpty(Access_Control_Allow_Origin))
                builder.Append("Access-Control-Allow-Origin :" + Access_Control_Allow_Origin + "\r\n");
        ...
        public void Send()
        {
            ...
            try
            {
                ...
                byte[] headerBytes = (this.Encoding ?? Encoding.UTF8).GetBytes(header);
                handler.Write(headerBytes, 0, headerBytes.Length);

                //发送空行
                byte[] lineBytes = (this.Encoding ?? Encoding.UTF8).GetBytes(Environment.NewLine);
